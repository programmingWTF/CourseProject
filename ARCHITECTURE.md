...
>
> 项目代码总量: 1445 行 (src/ 1271 行 + include/ 174 行)

用户在这个 GUI 中通过「搭积木」的方式组合流水线步骤（滤波 → 提取特征 → 显示），程序实时执行并在多个独立视口中对比效果。
...
│
├── include/                        ← 头文件（接口层, 174 行）
│   ├── FeatureBase.hpp       (20行) # 特征提取模板基类
│   ├── FeatureExtractors.hpp (32行) # 法线 / 曲率提取器
│   ├── FilterBase.hpp        (16行) # 滤波抽象基类
│   ├── Filters.hpp           (39行) # 三种滤波实现
│   ├── Logger.hpp            (18行) # 日志模块
│   ├── Pipeline.hpp          (35行) # 流水线编排
│   └── PointCloudIO.hpp      (14行) # 点云文件 IO
│
├── src/                            ← 源文件（实现层, 1271 行）
│   ├── main.cpp             (939行) # 主程序：GUI + 流水线执行引擎
│   ├── Filters.cpp           (60行) # 滤波实现（含公共模板 helper）
│   ├── FeatureExtractors.cpp (66行) # 特征提取实现
│   ├── Pipeline.cpp          (55行) # 流水线编排实现
│   ├── PointCloudIO.cpp     (108行) # 文件读写(.pcd/.ply/.bin)
│   └── Logger.cpp            (43行) # 日志实现
│
├── extern/imgui/                   # 内置 Dear ImGui (git submodule)
├── data/                           # 测试点云
...
| 流水线执行引擎      | `execute_pipeline_steps()` 独立函数，按序执行滤波 → 提取特征 → 截取快照 → 可选保存到文件 |
...
| 组件                    | 职责                                                                  |
| ----------------------- | --------------------------------------------------------------------- |
| `PointCloudPipeline`    | 编排滤波步骤 + 触发特征提取（法线 / 曲率），核心方法 `execute(cloud)` |
...
| 组件            | 职责                                                     |
| --------------- | -------------------------------------------------------- |
| `PointCloudIO`  | 文件读写（.pcd / .ply / .bin），含空路径和文件存在性校验 |
...
>
> `Filters.cpp` 内部使用匿名命名空间中的 `apply_pcl_filter<T>()` 模板函数抽取了「setInputCloud → filter → 替换指针」的公共模式，消除了三个滤波方法的重复代码。
...
>
> **法线复用机制**：`CurvatureExtractor` 新增 `setInputNormals()` 方法，允许外部注入预先计算好的法线。当流水线同时配置了法线和曲率提取器时，`PointCloudPipeline::execute()` 会将法线计算结果传入曲率提取器，避免重复计算法线，节省处理时间。
...
| ComputeCurvature                             | `CurvatureExtractor`（复用已算法线） | 在 pipeline->execute() 中自动执行                 |
...
| `getNormals()`              | 获取上次 `execute()` 的法线结果（含 assert 校验） |
| `getCurvatures()`           | 获取上次 `execute()` 的曲率结果（含 assert 校验） |
...
> 3. `getNormals()` / `getCurvatures()` 内部有 `assert(executed_)` 保护，防止在 `execute()` 之前误调用。
...
④ 如有 curvature_extractor_：
    · 如果 normals_ 已有数据 → setInputNormals(normals_) 复用
    · 否则 curvature_extractor_ 内部自行计算法线
    · 计算曲率
```

> 设计要点：法线与曲率在特征模块中是平级关系，但 Pipeline 层面通过法线复用避免了冗余计算。

### 4. PointCloudIO — 文件读写

```
load(path)    → pcl::PointCloud<PointXYZ>::Ptr
  · 空路径直接返回 nullptr 并打印错误
  · 文件不存在直接返回 nullptr 并打印错误
  · 支持自动检测扩展名

save<T>(path) → bool  (模板，支持 PointXYZ / PointNormal)
  · .pcd — PCL 原生格式
  · .ply — Stanford Polygon 格式
  · .bin — 读取时支持 KITTI 格式（每 4 float: xyz + intensity），保存时自动转为 .pcd
```
...
```
  ShowNormals 步骤
    ...
    │
    └─ { snapshot.cloud, snapshot.normals } → PipelineExecResult.snapshots
...
PipelineExecContext     → 流水线执行上下文（点云引用 + 输出路径 + 日志器）
PipelineExecResult      → 流水线执行结果（视图快照列表 + 处理前/后点数）
```
...
build_viewer_scene(viewer, original, config, snapshots) → 构建 PCL 多视口可视化场景

// === 特征合并 ===
build_featured_point_cloud(xyz, normals, curvatures, prev_featured_cloud) → featured_cloud
// 合并 XYZ + 法线 + 曲率 → PointNormal 云
// prev_featured_cloud 只在点索引一致时有效（滤波步骤改变了点索引后必须废弃）

// === 流水线执行引擎（独立函数，从 main() 中抽取） ===
execute_pipeline_steps(steps, ctx) → PipelineExecResult
// 遍历用户配置的步骤列表，构建 Pipeline 实例，执行滤波/特征提取/快照截取/文件保存
```
...
启动
 ...
 │   ├─ 用户点击「执行」时：
 │   │   │
 │   │   ├─ 构造 PipelineExecContext
 │   │   ├─ 调用 execute_pipeline_steps(steps, ctx)
 │   │   │   内部逻辑：
 │   │   │   ├─ 滤波步骤 → pipeline->addStage(filter)
 │   │   │   │    └─ 勾选保存? → flush → save → 新建 pipeline
...
执行过程：
  ...
  ComputeCurvature:
    ...
    Pipeline::execute() 内部检测到 normals_ 已有数据
      → curvature_extractor_->setInputNormals(normals_)  ← 复用，不重复计算
...
**数据同步**：唯一共享状态 `VisualizerData` 用 `std::mutex` 保护。主线程修改 `original_display` 和其他字段时同样持有该锁，确保无 data race。
...
主线程                         Visualizer 线程
  ...
  ├─ lock                         ├─ while（running）:
  │  写 data                       │    lock
  │  should_update = true          │    检查 should_update
  │  unlock                        │    如果是 → rebuild_scene()
  │                               │    unlock
...
1. **模块化** — 滤波、特征、IO、日志各司其职。加新滤波只需继承 `FilterBase`，加新特征只需继承 `FeatureBase<T>`。滤波的公共「setInputCloud → filter → 替换指针」模式已抽取为 `apply_pcl_filter<T>()` 模板函数。

2. **线程安全** — 唯一的共享状态 `VisualizerData` 用 `std::mutex` 保护，主线程和 Visualizer 线程的所有读写操作均在锁内进行。

3. **流水线隔离（仅限中途保存时）** — 当某步骤勾选「保存输出」时，会先执行已积压的步骤 → 保存 → **新建 Pipeline 实例**。这是因为 `execute()` 已消费了当前 pipeline 的执行上下文，后续步骤需要干净状态。不保存时全程只用一个 Pipeline，一气呵成。

4. **特征复用** — Pipeline 层面自动将法线计算结果传入曲率提取器，避免重复的 kd-tree 查询和法线估计。CurvatureExtractor 也支持独立运行（无外部法线时内部自动计算），保持接口兼容性。

5. **可扩展** — `FeatureBase` 模板已保留扩展到 `PointXYZRGB`、`PointXYZI` 的空间；`PointCloudIO::save<T>` 可处理任意点类型。

6. **对比友好** — 每步显示独立视口，原始点云固定左上角视口，随时对照。PassThrough 步骤的 UI 在 min >= max 时会显示红色警告。

7. **关注点分离** — 流水线执行逻辑已从 `main()` 中抽取为 `execute_pipeline_steps()` 独立函数，`main()` 只负责 UI 渲染和线程管理，代码更易测试和维护。
